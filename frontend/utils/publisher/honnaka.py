...
actor_url = "https://honnaka.jp/actress/"
actor_selector = "div.p-hoverCard"
actor_next_selector = "a[rel='next']"

movie_start_url = "https://honnaka.jp/actress/detail/17816"
...
def honnaka_actors_parser(item):
...
def honnaka_movies_parser(item):
        # print(item)
        link = item.get("href")
        return {
            "link": link,
...
honnaka_actor_detail_parser_dict = {
    "avatar": ['img.u-hidden--sp', "data-src"],
    "profile": ['div.table', "html"],
}

honnaka_actor_detail_additional_dict = {
    "detail_parsed": True
}
...
def update_document(doc):
    ...
    detail_result = parse_detail(
        url=detail_url,
        parse_dict=honnaka_actor_detail_parser_dict,
        additional_dict=honnaka_actor_detail_additional_dict
    )
...
def update_all_documents(collection_name="actress", selector = "honnaka",sleep_time = 1):
    # 连接 MongoDB
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db[collection_name]
    # 更新所有文档publisher为honnaka的文档
    count = 0
...
def get_movie_code_from_url(url):
    # https://honnaka.jp/works/detail/SOE245?page_from=actress&sys_code=6799
    # get -> SOE245
...
def update_all_documents_movies(selector = "honnaka"):
...
def download_movie_image_and_change_url_to_local(selector='honnaka', 
...
def download_actress_image_and_change_url_to_local(selector='honnaka', save_folder = r"\\10.16.12.105\disk\D\Pictures\J_IMG_LIB\actress"):
