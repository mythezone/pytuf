...
from ideapocket import * 

def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 ideapocket actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://ideapocket.com/actress/a",
        "https://ideapocket.com/actress/ka",
        "https://ideapocket.com/actress/sa",
        "https://ideapocket.com/actress/ta",
        "https://ideapocket.com/actress/na",
        "https://ideapocket.com/actress/ha",
        "https://ideapocket.com/actress/ma",
        "https://ideapocket.com/actress/ya",
        "https://ideapocket.com/actress/ra",
        "https://ideapocket.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "ideapocket", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> ideapocket_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "ideapocket", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="ideapocket",  # 只处理 publisher=ideapocket
