...
actor_url = "https://s1s1s1.com/actress/"
actor_selector = "div.p-hoverCard"
actor_next_selector = "a[rel='next']"

movie_start_url = "https://s1s1s1.com/actress/detail/17816"
...
s1s1s1_actor_detail_parser_dict = {
    "avatar": ['img.u-hidden--sp', "data-src"],
    "profile": ['div.table', "html"],
}

s1s1s1_actor_detail_additional_dict = {
    "detail_parsed": True
}
...
def update_document(doc):
    ...
    detail_result = parse_detail(
        url=detail_url,
        parse_dict=s1s1s1_actor_detail_parser_dict,
        additional_dict=s1s1s1_actor_detail_additional_dict
    )
...
def update_all_documents(collection_name="actress", selector = "s1s1s1",sleep_time = 1):
    # 连接 MongoDB
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db[collection_name]
    # 更新所有文档publisher为s1s1s1的文档
    count = 0
...
def get_movie_code_from_url(url):
    # https://s1s1s1.com/works/detail/SOE245?page_from=actress&sys_code=6799
    # get -> SOE245
...
def update_all_documents_movies(selector = "s1s1s1"):
...
def download_movie_image_and_change_url_to_local(selector='s1s1s1', 
...
def download_actress_image_and_change_url_to_local(selector='s1s1s1', save_folder = r"\\10.16.12.105\disk\D\Pictures\J_IMG_LIB\actress"):
