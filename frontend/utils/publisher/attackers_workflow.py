...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 attackers actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://attackers.net/actress/a",
        "https://attackers.net/actress/ka",
        "https://attackers.net/actress/sa",
        "https://attackers.net/actress/ta",
        "https://attackers.net/actress/na",
        "https://attackers.net/actress/ha",
        "https://attackers.net/actress/ma",
        "https://attackers.net/actress/ya",
        "https://attackers.net/actress/ra",
        "https://attackers.net/actress/wa"
    ]
    # 遍历所有起始URL
    for url in all_start_urls:
        ...
        get_list(
            ...
            parser=attackers_actors_parser,
            save=True,  # 入库
            key="link",  # 避免重复
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "attackers", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> attackers_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "attackers", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="attackers",  # 只处理 publisher=attackers
