...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 wanz actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://wanz-factory.com/actress/a",
        "https://wanz-factory.com/actress/ka",
        "https://wanz-factory.com/actress/sa",
        "https://wanz-factory.com/actress/ta",
        "https://wanz-factory.com/actress/na",
        "https://wanz-factory.com/actress/ha",
        "https://wanz-factory.com/actress/ma",
        "https://wanz-factory.com/actress/ya",
        "https://wanz-factory.com/actress/ra",
        "https://wanz-factory.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "wanz", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> wanz_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "wanz", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="wanz",  # 只处理 publisher=wanz
