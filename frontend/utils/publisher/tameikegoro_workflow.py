...
from tameikegoro import * 

def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 tameikegoro actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://tameikegoro.jp/actress/a",
        "https://tameikegoro.jp/actress/ka",
        "https://tameikegoro.jp/actress/sa",
        "https://tameikegoro.jp/actress/ta",
        "https://tameikegoro.jp/actress/na",
        "https://tameikegoro.jp/actress/ha",
        "https://tameikegoro.jp/actress/ma",
        "https://tameikegoro.jp/actress/ya",
        "https://tameikegoro.jp/actress/ra",
        "https://tameikegoro.jp/actress/wa"
    ]
    # 遍历所有起始URL
    for url in all_start_urls:
        ...
        get_list(
            ...
            parser=tameikegoro_actors_parser,
            save=True,  # 入库
            key="link",  # 避免重复
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "tameikegoro", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> tameikegoro_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "tameikegoro", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="tameikegoro",  # 只处理 publisher=tameikegoro
