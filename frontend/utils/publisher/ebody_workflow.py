...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 ebody actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://av-e-body.com/actress/a",
        "https://av-e-body.com/actress/ka",
        "https://av-e-body.com/actress/sa",
        "https://av-e-body.com/actress/ta",
        "https://av-e-body.com/actress/na",
        "https://av-e-body.com/actress/ha",
        "https://av-e-body.com/actress/ma",
        "https://av-e-body.com/actress/ya",
        "https://av-e-body.com/actress/ra",
        "https://av-e-body.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "ebody", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> ebody_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "ebody", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="ebody",  # 只处理 publisher=ebody
