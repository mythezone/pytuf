...
actor_url = "https://tameikegoro.jp/actress/"
actor_selector = "div.p-hoverCard"
actor_next_selector = "a[rel='next']"

movie_start_url = "https://tameikegoro.jp/actress/detail/17816"
...
def tameikegoro_actors_parser(item):
...
def tameikegoro_movies_parser(item):
        # print(item)
        link = item.get("href")
        return {
            "link": link,
...
tameikegoro_actor_detail_parser_dict = {
    "avatar": ['img.u-hidden--sp', "data-src"],
    "profile": ['div.table', "html"],
}

tameikegoro_actor_detail_additional_dict = {
    "detail_parsed": True
}
...
def get_movie_list_by_actor(url):
    list_results = get_list(
        ...
        parser=tameikegoro_movies_parser,
        save=False,
    )
    return list_results
...
def update_document(doc):
    ...
    detail_result = parse_detail(
        url=detail_url,
        parse_dict=tameikegoro_actor_detail_parser_dict,
        additional_dict=tameikegoro_actor_detail_additional_dict
    )
...
def update_all_documents(collection_name="actress", selector = "tameikegoro",sleep_time = 1):
    # 连接 MongoDB
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db[collection_name]
    # 更新所有文档publisher为tameikegoro的文档
    count = 0
...
def get_movie_code_from_url(url):
    # https://tameikegoro.jp/works/detail/SOE245?page_from=actress&sys_code=6799
    # get -> SOE245
...
def update_all_documents_movies(selector = "tameikegoro"):
...
def download_movie_image_and_change_url_to_local(selector='tameikegoro', 
...
def download_actress_image_and_change_url_to_local(selector='tameikegoro', save_folder = r"\\10.16.12.105\disk\D\Pictures\J_IMG_LIB\actress"):
