...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 honnaka actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://honnaka.jp/actress/a",
        "https://honnaka.jp/actress/ka",
        "https://honnaka.jp/actress/sa",
        "https://honnaka.jp/actress/ta",
        "https://honnaka.jp/actress/na",
        "https://honnaka.jp/actress/ha",
        "https://honnaka.jp/actress/ma",
        "https://honnaka.jp/actress/ya",
        "https://honnaka.jp/actress/ra",
        "https://honnaka.jp/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "honnaka", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> honnaka_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "honnaka", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="honnaka",  # 只处理 publisher=honnaka
