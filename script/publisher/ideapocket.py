...
actor_url = "https://ideapocket.com/actress/"
actor_selector = "div.p-hoverCard"
actor_next_selector = "a[rel='next']"

movie_start_url = "https://ideapocket.com/actress/detail/17816"
...
def ideapocket_actors_parser(item):
...
def ideapocket_movies_parser(item):
        # print(item)
        link = item.get("href")
        return {
            "link": link,
...
ideapocket_actor_detail_parser_dict = {
    "avatar": ['img.u-hidden--sp', "data-src"],
    "profile": ['div.table', "html"],
}

ideapocket_actor_detail_additional_dict = {
    "detail_parsed": True
}
...
def get_movie_list_by_actor(url):
    list_results = get_list(
        ...
        parser=ideapocket_movies_parser,
        save=False,
    )
    return list_results
...
def update_document(doc):
    ...
    detail_result = parse_detail(
        url=detail_url,
        parse_dict=ideapocket_actor_detail_parser_dict,
        additional_dict=ideapocket_actor_detail_additional_dict
    )
...
def update_all_documents(collection_name="actress", selector = "ideapocket",sleep_time = 1):
    # 连接 MongoDB
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db[collection_name]
    # 更新所有文档publisher为ideapocket的文档
    count = 0
...
def get_movie_code_from_url(url):
    # https://ideapocket.com/works/detail/SOE245?page_from=actress&sys_code=6799
    # get -> SOE245
...
def update_all_documents_movies(selector = "ideapocket"):
...
def download_movie_image_and_change_url_to_local(selector='ideapocket', 
...
def download_actress_image_and_change_url_to_local(selector='ideapocket', save_folder = r"\\10.16.12.105\disk\D\Pictures\J_IMG_LIB\actress"):
...
if __name__ == "__main__":
    ...
    # 可选的解析器函数，假设我们只想获取链接和标题等
    
    # for name in names:
    #     actor_start_url = f"{actor_url}{name}"
    #     print("Start to crawl: ", actor_start_url)
    #     list_results = get_list(
    #         url=actor_start_url,
    #         item_selector=actor_selector,
    #         next_selector=actor_next_selector,
    #         parser=ideapocket_actors_parser,
    #         save=True,
    #         key="link",
    ...
    # # 假设我们要解析第一个列表项的 link
    # if list_results:
    #     first_item = list_results[0]
    #     if "link" in first_item and first_item["link"]:
    #         detail_url = first_item["link"]
    #         detail_result = parse_detail(
    #             url=detail_url,
    #             parse_dict=parse_dict,
