...
from fitch_av import * 

def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 fitch_av actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://fitch-av.com/actress/a",
        "https://fitch-av.com/actress/ka",
        "https://fitch-av.com/actress/sa",
        "https://fitch-av.com/actress/ta",
        "https://fitch-av.com/actress/na",
        "https://fitch-av.com/actress/ha",
        "https://fitch-av.com/actress/ma",
        "https://fitch-av.com/actress/ya",
        "https://fitch-av.com/actress/ra",
        "https://fitch-av.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "fitch_av", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> fitch_av_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "fitch_av", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="fitch_av",  # 只处理 publisher=fitch_av
