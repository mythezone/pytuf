...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 moodyz actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://moodyz.com/actress/a",
        "https://moodyz.com/actress/ka",
        "https://moodyz.com/actress/sa",
        "https://moodyz.com/actress/ta",
        "https://moodyz.com/actress/na",
        "https://moodyz.com/actress/ha",
        "https://moodyz.com/actress/ma",
        "https://moodyz.com/actress/ya",
        "https://moodyz.com/actress/ra",
        "https://moodyz.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "moodyz", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> moodyz_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "moodyz", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="moodyz",  # 只处理 publisher=moodyz
