...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 madonna actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://madonna-av.com/actress/a",
        "https://madonna-av.com/actress/ka",
        "https://madonna-av.com/actress/sa",
        "https://madonna-av.com/actress/ta",
        "https://madonna-av.com/actress/na",
        "https://madonna-av.com/actress/ha",
        "https://madonna-av.com/actress/ma",
        "https://madonna-av.com/actress/ya",
        "https://madonna-av.com/actress/ra",
        "https://madonna-av.com/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "madonna", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> madonna_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "madonna", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="madonna",  # 只处理 publisher=madonna
