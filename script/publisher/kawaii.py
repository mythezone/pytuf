...
actor_url = "https://kawaiikawaii.jp/actress/"
actor_selector = "div.p-hoverCard"
actor_next_selector = "a[rel='next']"

movie_start_url = "https://kawaiikawaii.jp/actress/detail/17816"
...
kawaii_actor_detail_parser_dict = {
    "avatar": ['img.u-hidden--sp', "data-src"],
    "profile": ['div.table', "html"],
}

kawaii_actor_detail_additional_dict = {
    "detail_parsed": True
}
...
def update_document(doc):
    ...
    detail_result = parse_detail(
        url=detail_url,
        parse_dict=kawaii_actor_detail_parser_dict,
        additional_dict=kawaii_actor_detail_additional_dict
    )
...
def update_all_documents(collection_name="actress", selector = "kawaii",sleep_time = 1):
    # 连接 MongoDB
    client = MongoClient(mongo_url)
    db = client[db_name]
    collection = db[collection_name]
    # 更新所有文档publisher为kawaii的文档
    count = 0
...
def get_movie_code_from_url(url):
    # https://kawaiikawaii.jp/works/detail/SOE245?page_from=actress&sys_code=6799
    # get -> SOE245
...
def update_all_documents_movies(selector = "kawaii"):
...
def download_movie_image_and_change_url_to_local(selector='kawaii', 
...
def download_actress_image_and_change_url_to_local(selector='kawaii', save_folder = r"\\10.16.12.105\disk\D\Pictures\J_IMG_LIB\actress"):
