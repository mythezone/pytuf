...
def crawl_and_save_actress_list():
    """
    阶段1：爬取所有 kawaii actress 列表并存储到 'actress_collection'.
    """
    all_start_urls = [
        "https://kawaiikawaii.jp/actress/a",
        "https://kawaiikawaii.jp/actress/ka",
        "https://kawaiikawaii.jp/actress/sa",
        "https://kawaiikawaii.jp/actress/ta",
        "https://kawaiikawaii.jp/actress/na",
        "https://kawaiikawaii.jp/actress/ha",
        "https://kawaiikawaii.jp/actress/ma",
        "https://kawaiikawaii.jp/actress/ya",
        "https://kawaiikawaii.jp/actress/ra",
        "https://kawaiikawaii.jp/actress/wa"
    ]
    # 遍历所有起始URL
...
def parse_actress_detail_and_update():
    ...
    """
    coll = db["actress"]
    for doc in coll.find({"publisher": "kawaii", "detail_parsed": {"$ne": True}}):
        updated_doc = update_document(doc)  # 内部会 parse_detail -> kawaii_actor_detail_parser_dict -> get_movie_list_by_actor
        if updated_doc:
...
def parse_movies_by_actress():
    ...
    for doc in coll_actress.find({"publisher": "kawaii", "movies_parsed": {"$ne": True}, "detail_parsed": True}):
...
def download_all_movies_screenshots():
    ...
    try:
        result = download_movie_image_and_change_url_to_local(
            selector="kawaii",  # 只处理 publisher=kawaii
