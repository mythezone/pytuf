...
def _get_proxies(
    controller_url: str = "http://127.0.0.1:9097", secret: Optional[str] = None
) -> Dict:
    resp = requests.get(
        f"{controller_url.rstrip('/')}/proxies", headers=_headers(secret), timeout=5
    )
    resp.raise_for_status()
    return resp.json() or {}


def _pick_selector_group(
    proxies_json: Dict, preferred: Optional[str] = None
) -> Optional[str]:
...
if __name__ == "__main__":
    ...
    secret_key = 12345678  # 如果未设置密钥，则设为 None
    group = "GLOBAL"  # 可指定 group，例如 'GLOBAL' 或 '🔰 节点选择'

    nodes = get_all_proxy_name(
        controller_url=controller, secret=secret_key, group_name=group
    )
